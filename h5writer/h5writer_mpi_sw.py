...
def _import_mpi4py():
    ...
    try:
        ...
        if StrictVersion(mpi4py.__version__) < StrictVersion(mpi4py_version_min):
            log_warning(
                logger,
                "Version of mpi4py is too old (currently installed: %s). Please install at least version %s or more recent."
                % (mpi4py.__version__, mpi4py_version_min),
            )
            MPI = None
        else:
            try:
...
class H5WriterMPISW(AbstractH5Writer):
    ...
    def __init__(self, filename, comm, chunksize=100, compression=None):
        ...
        if not isinstance(self.comm, MPI.Comm):
            log_and_raise_error(
                logger,
                "Cannot initialise H5WriterMPI instance. '%s' is not an mpi4py.MPI.Comm instance."
                % str(self.comm),
            )
        AbstractH5Writer.__init__(
            self, filename, chunksize=chunksize, compression=compression
        )
        self._closed = False
        if self._is_master():
            if os.path.exists(self._filename):
                log_warning(
                    logger,
                    self._log_prefix
                    + "File %s exists and is being overwritten" % (self._filename),
                )
    ...
    def _master_loop(self):
        ...
        while True:
            ...
            if source == 0:
                logger.warning(
                    "Received write package from master process! Skipping writing."
                )
                print(l)
                continue
            ...
            if l == "close":
                ...
                if "write_slice" in l:
                    ...
                    self._write_slice_master(
                        l["write_slice"], i=self._i
                    )  # slices[source]*(self.comm.size-1)+source-1)
                    self._i += 1
                    slices[source] += 1
                    # --
                    ...
                    log_info(
                        logger,
                        "Writing rate %.1f Hz; slice %i (writing %.4f sec; waiting %.4f sec, receiving %.4f sec)"
                        % (
                            slices.sum() / (time.time() - t_start),
                            self._i - 1,
                            t_write,
                            t_wait,
                            t_recv,
    ...
    def _transfer_numpy_arrays(self, skeleton, data_dict=None, source=None):
        ...
        for k in keys:
            ...
            if isinstance(skeleton[k], dict):
                self._transfer_numpy_arrays(
                    skeleton[k],
                    None if mode == "master" else data_dict[k],
                    source=source,
                )
                ...
                if mode == "master":
                    ...
                    skeleton[k] = numpy.empty(
                        shape=skeleton[k].shape, dtype=skeleton[k].dtype
                    )
                    ...
                    if numpy.dtype(data_dict[k].dtype) == numpy.float16:
                        raise RuntimeError("Data type float16 not supported")
                    self.comm.Send(
                        numpy.ascontiguousarray(data_dict[k]).data, dest=0, tag=0
                    )
    ...
    def close(self, barrier=True):
        ...
        if self._is_master():
            log_info(
                logger,
                self._log_prefix
                + "HDF5 writer master process for file %s closed." % (self._filename),
            )
        self._closed = True
