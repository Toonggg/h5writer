...
def main():
    ...
    if MPI is not None and MPI.COMM_WORLD.size > 1:
        Ws.append(
            H5WriterMPISW(filename=filename_mpisw, comm=MPI.COMM_WORLD, chunksize=3)
        )
    ...
    for W in Ws:
        ...
        if isinstance(W, H5Writer) or (isinstance(W, H5WriterMPISW) and not master):
            ...
            for i in range(N_frames):
                ...
                O["entry_1"] = {}
                O["entry_1"]["data_1"] = {}
                O["entry_1"]["data_1"]["ones"] = numpy.ones(
                    shape=(N_ones, N_ones), dtype=numpy.float32
                )
                O["entry_1"]["data_2"] = {}
    ...
    if master:
        with h5py.File(filename_no_mpi, "r") as f:
            ...
            assert (numpy.array(f["/entry_1/data_1/ones"]) == 1.0).all()
            assert (numpy.array(f["/entry_1/data_2/zeros"]) == 0.0).all()
